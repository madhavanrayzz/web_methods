...
def checkBypass(self, oldStatusCode, newStatusCode, oldContent,
    ...
    # If status code changed, auth is enforced → return NEW status
    if oldStatusCode != newStatusCode:
        return str(newStatusCode)

    # Same status code → deeper inspection
    auth_enforced = 0
    if len(filters) > 0:
        auth_enforced = auth_enforced_via_enforcement_detectors(
            self, filters, requestResponse, andOrEnforcement
        )

    # Always return REAL status code instead of labels
    if auth_enforced:
        return str(newStatusCode)
    elif oldContent == newContent:
        return str(oldStatusCode)
    else:
        return str(newStatusCode)
